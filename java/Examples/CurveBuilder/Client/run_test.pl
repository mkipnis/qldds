...
if ($debug ne '0') {
    ...
    $client_opts .= " $debug_opt -ORBLogFile client.log";
...
unlink $dcpsrepo_ior;
unlink qw/client.log sub.log DCPSInfoRepo.log/;
...
PerlACE::add_lib_path("$QLDDS_ROOT/Examples/CurveBuilder/CurveBuilderIDL/");
PerlACE::add_lib_path("$QLDDS_ROOT/lib/");
PerlACE::add_lib_path("$QLDDS_ROOT/java/qldds_utils/");

my $PUB = new PerlDDS::Process_Java('Client', $client_opts,
            ["$QLDDS_ROOT/lib/".'ratehelpers.jar',
            "$QLDDS_ROOT/lib/".'piecewiseyieldcurve.jar',
	    "$QLDDS_ROOT/Examples/CurveBuilder/CurveBuilderIDL/".'CurveBuilder.jar',
	    "$QLDDS_ROOT/lib/".'qldds_utils_java.jar']);
...
if ($PublisherResult != 0) {
    print STDERR "ERROR: clientlisher returned $PublisherResult\n";
    $status = 1;
}
